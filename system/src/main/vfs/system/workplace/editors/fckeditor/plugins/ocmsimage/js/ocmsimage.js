...
// get the span around the image, if present
var oSpan = null;
...
function LoadSelection()
{
    ...
    var imgHSp = "";
    var imgVSp = "";
    if (FCK.Selection.HasAncestorNode("SPAN") || FCK.Selection.HasAncestorNode("TABLE")) {
        if (FCK.Selection.HasAncestorNode("SPAN")) {
            oSpan = FCK.Selection.MoveToAncestorNode("SPAN");
        } else {
            oSpan = FCK.Selection.MoveToAncestorNode("TABLE");
        }
        try {
...
function Ok()
{
    ...
    if (showEnhancedOptions && oLink && (insertSubTitle() || insertCopyright())) {
        // original link has to be removed if a div is created in enhanced options
    ...
    if (showEnhancedOptions && oSpan != null && (oSpan.id.substring(0, 5) == "aimg_" || oSpan.id.substring(0, 5) == "timg_")) {
        // div is already present, select it
        FCK.Selection.SelectNode(oSpan);
        // remove child elements of div
    ...
    if (!imgCreated) {
        // delete the selection (either the image or the complete div) if the image was not freshly created
        ...
        if (!oEditor.FCKBrowserInfo.IsIE) {
            ...
            // in IE... insert the new element to make sure the div is inserted
            oImage = oEditor.FCK.InsertElementAndGetIt(oImage);
        }
    }
...
function createEnhancedImage() {
    if (isEnhancedPreview()) {
        ...
        SetAttribute(oNewElement, "id", "aimg_" + activeImage.hashCode);
        ...
        if (insertCopyright()) {
            ...
            oSpan2.id = "cimg_" + activeImage.hashCode;
        ...
        if (insertSubTitle()) {
            ...
            oSpan3.id = "simg_" + activeImage.hashCode;
...
function UpdatePreview(useTimeout)
{
    ...
    else
    {
        ...
        if (useTimeout && oSpan != null && oSpan.id == "aimg_" + activeImage.hashCode) {
